...
export default function ContactHeader() {
  const { tFn, lang, isRTL } = useLanguage();
  ...
  const titleSpan = tr(
    "contact.titleSpan",
    lang === "he" ? "צור קשר" : lang === "en" ? "Contact" : "تواصل"
  );

  const titleMain = tr(
    "contact.title",
    lang === "he" ? "איתנו" : lang === "en" ? "Us" : "معنا"
  );

  const subtitle = tr(
    "contact.subtitle",
    lang === "he"
      ? "נשמח לשמוע מכם בכל זמן — הצוות שלנו כאן לעזור."
      : lang === "en"
      ? "We’re happy to hear from you anytime — our team is ready to help."
      : "يسعدنا تواصلك معنا في أي وقت، فريقنا جاهز لخدمتك."
  );

  return (
    <div
      className="relative text-center max-w-3xl mx-auto mb-8 sm:mb-12"
      dir={isRTL ? "rtl" : "ltr"}
    >
      <h2 className="font-black leading-tight tracking-tight">
        <span className="inline-flex items-center justify-center rounded-[20px] sm:rounded-[22px] bg-white ring-1 ring-gray-200/90 shadow-[0_8px_24px_rgba(6,24,44,.06)] px-4 sm:px-6 py-2">
          <span className="bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 bg-clip-text text-transparent text-[34px] sm:text-[44px]">
            {titleSpan}
          </span>
